...
if [ "${oldMD5}" == "${newMD5}" ]; then
  ...
  if [ ${DEBUG} == "yes" ]; then
    if [ ${UNIT_TEST} == "yes" ]; then
      cmake ../.. -DTHREAD_TEST=YES -DUNIT_TEST=YES || (echo "cmake失败！" && exit)
    else
      cmake ../.. -DTHREAD_TEST=YES || (echo "cmake失败！" && exit)
    fi
  else
  ...
  # 保存新的 MD5
  touch CMakeLists.txt.md5 || (echo LogData.cpp"有问题！" && exit)
